// Database Awal Default jika server pertama kali diakses oleh klien baru
const defaultData = {
    akreditasi: [
...
// State manager sinkronisasi data localstorage
...
// Render data ke baris-baris tabel secara asinkronus
function renderAllTables() {
    ...
    tbodyAkre.innerHTML = appData.akreditasi.map((item, idx) => `
        <tr class="hover:bg-gray-50 border-b border-gray-200">
            ...
            <td class="p-3 text-center"><span class="px-2 py-0.5 rounded text-[11px] font-bold ${item.nilai === 'Belum' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-800'}">${item.nilai}</span></td>
            <td class="p-3 text-center">${item.tahun}</td>
            <td class="p-3 text-center font-semibold text-emerald-600">${item.masa}</td>
            <td class="p-3 text-gray-600 text-[11px]">${item.sk}</td>
            <td class="p-3 text-center space-x-3 non-printable text-sm">
                <button onclick="editData('akreditasi', ${idx})" class="text-blue-600 hover:text-blue-800 cursor-pointer" title="Edit"><i class="fa fa-edit"></i></button>
                <button onclick="deleteData('akreditasi', ${idx})" class="text-red-500 hover:text-red-700 cursor-pointer" title="Hapus"><i class="fa fa-trash"></i></button>
            </td>
        </tr>
    `).join('');
    ...
    tbodyOps.innerHTML = appData.operasional.map((item, idx) => `
        <tr class="hover:bg-gray-50 border-b border-gray-200">
            ...
            <td class="p-3 text-center"><span class="px-2 py-0.5 rounded text-[11px] font-bold ${item.habis === '0' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}">${item.habis}</span></td>
            <td class="p-3 text-gray-600 text-[11px]">${item.sk}</td>
            <td class="p-3 text-center space-x-3 non-printable text-sm">
                <button onclick="editData('operasional', ${idx})" class="text-blue-600 hover:text-blue-800 cursor-pointer"><i class="fa fa-edit"></i></button>
                <button onclick="deleteData('operasional', ${idx})" class="text-red-500 hover:text-red-700 cursor-pointer"><i class="fa fa-trash"></i></button>
            </td>
        </tr>
    `).join('');
    ...
    tbodyPeserta.innerHTML = appData.peserta.map((item, idx) => `
        <tr class="hover:bg-gray-50 border-b border-gray-200 text-center">
            <td class="p-3 text-left font-medium">${idx + 1}</td>
            ...
            <td class="p-3 font-bold text-blue-600 text-left">${item.prog}</td>
            ...
            <td class="p-3 space-x-3 non-printable text-sm">
                <button onclick="editData('peserta', ${idx})" class="text-blue-600 hover:text-blue-800 cursor-pointer"><i class="fa fa-edit"></i></button>
                <button onclick="deleteData('peserta', ${idx})" class="text-red-500 hover:text-red-700 cursor-pointer"><i class="fa fa-trash"></i></button>
            </td>
        </tr>
    `).join('');

    // Tab 5: Program Layanan KKNI
    const tbodyProg = document.querySelector("#table-program tbody");
    tbodyProg.innerHTML = appData.program.map((item, idx) => {
        const renderBadge = (v) => v > 0 ? `<span class="bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded font-bold">${v}%</span>` : `<span class="text-gray-300">0</span>`;
        return `
            <tr class="hover:bg-gray-50 border-b border-gray-200 text-center">
                <td class="p-3 text-left font-medium">${idx + 1}</td>
                ...
                <td class="p-2 border-l border-gray-100">${renderBadge(item.l1)}</td><td>${renderBadge(item.l2)}</td><td>${renderBadge(item.l3)}</td>
                <td>${renderBadge(item.l4)}</td><td>${renderBadge(item.l5)}</td><td>${renderBadge(item.l6)}</td>
                <td>${renderBadge(item.l7)}</td><td>${renderBadge(item.l8)}</td><td>${renderBadge(item.l9)}</td>
                ...
                <td class="p-3 space-x-3 non-printable text-sm">
                    <button onclick="editData('program', ${idx})" class="text-blue-600 hover:text-blue-800 cursor-pointer"><i class="fa fa-edit"></i></button>
                    <button onclick="deleteData('program', ${idx})" class="text-red-500 hover:text-red-700 cursor-pointer"><i class="fa fa-trash"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
// Rekalkulasi data ringkasan agregat
function calculateStats() {
    const totalLkp = appData.akreditasi.length;
    const terakreditasi = appData.akreditasi.filter(i => ['A', 'B', 'C'].includes(i.nilai)).length;
    ...
    let totalPd = 0;
    appData.peserta.forEach(i => totalPd += (Number(i.sl) + Number(i.sp) + Number(i.pl) + Number(i.pp)));
...
function openModal(type, index = -1) {
    ...
    if (type === 'akreditasi') {
        ...
        html = `
            <div><label class="block font-bold mb-1">NPSN</label><input type="text" id="f-npsn" value="${item.npsn||''}" class="w-full p-2 border rounded" required></div>
            <div><label class="block font-bold mb-1">Nama Lembaga</label><input type="text" id="f-nama" value="${item.nama||''}" class="w-full p-2 border rounded" required></div>
            <div><label class="block font-bold mb-1">Alamat Lokasi</label><input type="text" id="f-alamat" value="${item.alamat||''}" class="w-full p-2 border rounded"></div>
            ...
            <div><label class="block font-bold mb-1">Habis Masa</label><input type="number" id="f-habis" value="${item.habis||'2029'}" class="w-full p-2 border rounded"></div>
        ...
        html = `
            ...
            <div><label class="block font-bold mb-1">Program Kursus</label><input type="text" id="f-prog" value="${item.prog||''}" class="w-full p-2 border rounded"></div>
            ...
            <div><label class="block font-bold mb-1">Status Verval</label><select id="f-verval" class="w-full p-2 border rounded"><option value="Sudah" ${item.verval==='Sudah'?'selected':''}>Sudah</option><option value="Belum" ${item.verval==='Belum'?'selected':''}>Belum</option></select></div>
        `;
    } else if (type === 'program') {
        html = `
            <div><label class="block font-bold mb-1">NPSN</label><input type="text" id="f-npsn" value="${item.npsn||''}" class="w-full p-2 border rounded" required></div>
            <div><label class="block font-bold mb-1">Nama Lembaga</label><input type="text" id="f-nama" value="${item.nama||''}" class="w-full p-2 border rounded" required></div>
            <div class="sm:col-span-2"><label class="block font-bold mb-1">Alamat</label><input type="text" id="f-alamat" value="${item.alamat||''}" class="w-full p-2 border rounded"></div>
            <div><label class="block font-bold mb-1">Level 1 (%)</label><input type="number" id="f-l1" value="${item.l1||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 2 (%)</label><input type="number" id="f-l2" value="${item.l2||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 3 (%)</label><input type="number" id="f-l3" value="${item.l3||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 4 (%)</label><input type="number" id="f-l4" value="${item.l4||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 5 (%)</label><input type="number" id="f-l5" value="${item.l5||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 6 (%)</label><input type="number" id="f-l6" value="${item.l6||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 7 (%)</label><input type="number" id="f-l7" value="${item.l7||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 8 (%)</label><input type="number" id="f-l8" value="${item.l8||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Level 9 (%)</label><input type="number" id="f-l9" value="${item.l9||0}" class="w-full p-2 border rounded" max="100"></div>
            <div><label class="block font-bold mb-1">Label Keterangan</label><input type="text" id="f-text" value="${item.text||'LEVEL 1'}" class="w-full p-2 border rounded"></div>
        `;
    }
...
// Ambil & Simpan Form Data (Aksi Gabungan Create & Update)
function saveFormData(e) {
    ...
    if (index > -1) {
        appData[type][index] = dataObj; // Jalankan Update
    } else {
        appData[type].push(dataObj); // Jalankan Create
    }
...
function deleteData(type, index) {
    if (confirm(`Apakah Anda yakin ingin menghapus data baris ke-${index + 1} pada tabel ${type}?`)) {
...
// Parser Data Statistik Grafik
function getAkreditasiStats() {
    let counts = { A: 0, B: 0, C: 0, Belum: 0 };
    appData.akreditasi.forEach(item => { if (counts[item.nilai] !== undefined) counts[item.nilai]++; });
    return [counts.A, counts.B, counts.C, counts.Belum];
}

function getGenderStats() {
    let l = 0, p = 0;
    appData.peserta.forEach(i => { l += (Number(i.sl) + Number(i.pl)); p += (Number(i.sp) + Number(i.pp)); });
    return [l, p];
}
...
function initCharts() {
    charts.bar = new Chart(document.getElementById('barChartAkreditasi').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{ data: getAkreditasiStats(), backgroundColor: ['#10b981', '#3b82f6', '#f59e0b', '#ef4444'], borderRadius: 4 }]
        },
...
function updateCharts() {
    if (charts.bar) { charts.bar.data.datasets[0].data = getAkreditasiStats(); charts.bar.update(); }
    if (charts.gender) { charts.gender.data.datasets[0].data = getGenderStats(); charts.gender.update(); }
    if (charts.verval) { charts.verval.data.datasets[0].data = getVervalStats(); charts.verval.update(); }
}
